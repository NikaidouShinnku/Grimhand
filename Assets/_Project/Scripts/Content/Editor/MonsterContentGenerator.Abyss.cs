...
namespace Grimhand.Content.Editor
{
    public static partial class MonsterContentGenerator
    {
        ...
        static AbyssCards CreateAbyssCards() =>
            new()
            {
                TideLance = SaveCard("m_tide_lance", "枪刺", "char_seahorse_guard", 1, CardType.Attack, null,
                    Action(EffectActionType.DealDamage, EffectTarget.DefaultEnemy, 12)),
                TideCharge = SaveCard("m_tide_charge", "浪潮冲锋", "char_seahorse_guard", 2, CardType.Attack, null,
                    CardRarity.Rare,
                    Action(EffectActionType.DealDamage, EffectTarget.DefaultEnemy, 12,
                ...
                WaterShield = SaveCard("m_water_shield", "以水为盾", "char_seahorse_guard", 2, CardType.Defense, Kw("parry"),
                    ...
                    Action(EffectActionType.ApplyStatus, EffectTarget.LastActionActor, 0,
                        ...
                        condition: ReactionConditionType.LastActionAttackOnSelf)),
                GuardStance = SaveCard("m_guard_stance", "守卫姿态", "char_seahorse_guard", 1, CardType.Defense, null,
                ...
                FinalGuard = SaveCard("m_final_guard", "终焉守护", "char_seahorse_guard", 4, CardType.Defense, Kw("parry"),
                    ...
                    Action(EffectActionType.ReducePlayerEnergyRegenNextTurn, EffectTarget.Self, 99,
                        condition: ReactionConditionType.LastActionAttackOnSelf)),
                JellySting = SaveCard("m_jelly_sting", "电刺击", "char_jellyfish_caster", 1, CardType.Attack, null,
                    Action(EffectActionType.DealDamage, EffectTarget.DefaultEnemy, 12)),
                PhaseCurrent = SaveCard("m_phase_current", "相位电流", "char_jellyfish_caster", 2, CardType.Status, Kw("slow"),
                    CardRarity.Rare,
                    Action(EffectActionType.SwapRandomEnemies, EffectTarget.DefaultEnemy, 1,
                        statusId: StatusCatalog.Slow, stacks: 1, duration: 2)),
                GelWall = SaveCard("m_gel_wall", "凝胶护壁", "char_jellyfish_caster", 2, CardType.Defense, null,
                    Action(EffectActionType.GainBlock, EffectTarget.Self, 8)),
                BounceSting = SaveCard("m_bounce_sting", "弹射蛰刺", "char_jellyfish_caster", 3, CardType.Attack, null,
                    CardRarity.Rare,
                    Action(EffectActionType.DealDamage, EffectTarget.DefaultEnemy, 8, splashBehind: true),
                    Action(EffectActionType.ApplyStatus, EffectTarget.DefaultEnemy, 0,
                        statusId: StatusCatalog.Poison, stacks: 5, duration: -1, splashBehind: true)),
                ...
                FinalSummon = SaveCard("m_final_summon", "终焉召唤", "char_jellyfish_caster", 4, CardType.Status,
                    ...
                    Action(EffectActionType.ApplyStatus, EffectTarget.Self, 0,
                        statusId: StatusCatalog.FinalSummonPending, stacks: 1, duration: 3)),
                ParalyzeSting = SaveCard("m_paralyze_sting", "麻痹之电", "char_jellyfish_caster", 3, CardType.Attack,
                    null, CardRarity.Rare,
                    Action(EffectActionType.DealDamage, EffectTarget.DefaultEnemy, 15),
                    Action(EffectActionType.SwapTargetWithBehind, EffectTarget.DefaultEnemy, 0)),
                MermaidSlash = SaveCard("m_mermaid_slash", "劈砍", "char_mermaid_warrior", 1, CardType.Attack, null,
                    Action(EffectActionType.DealDamage, EffectTarget.DefaultEnemy, 11)),
                TidalPower = SaveCard("m_tidal_power", "潮汐之力", "char_mermaid_warrior", 3, CardType.Status, null,
                    CardRarity.Rare,
                    Action(EffectActionType.ApplyStatus, EffectTarget.Self, 0,
                        statusId: StatusCatalog.AttackUpPercent, stacks: 30, duration: 3),
                    Action(EffectActionType.ApplyStatus, EffectTarget.Self, 0,
                        statusId: StatusCatalog.MermaidTidalCostCut, stacks: 1, duration: 2)),
                MermaidShield = SaveCard("m_mermaid_shield", "举盾", "char_mermaid_warrior", 2, CardType.Defense, null,
                    Action(EffectActionType.GainBlock, EffectTarget.Self, 10)),
                WaveCleave = SaveCard("m_wave_cleave", "破浪斩", "char_mermaid_warrior", 2, CardType.Attack, null,
                    CardRarity.Rare,
                    Action(EffectActionType.DealDamage, EffectTarget.DefaultEnemy, 15,
                        reach: TargetReach.Any)),
                AbyssLash = SaveCard("m_abyss_lash", "深渊鞭笞", MinionTraitCatalog.AbyssCreatureCharacterId, 1,
                    CardType.Attack, null, CardRarity.Rare,
                    Action(EffectActionType.DealDamage, EffectTarget.DefaultEnemy, 5,
                        reach: TargetReach.Any, hitCount: 3)),
                ...
                ShellCraft = SaveCard("m_shell_craft", "制造外壳", MinionTraitCatalog.AbyssCreatureCharacterId, 2,
                    CardType.Defense, null,
                    Action(EffectActionType.GainBlock, EffectTarget.Self, 11)),
                CorrosionVolley = SaveCard("m_corrosion_volley", "腐蚀乱射", MinionTraitCatalog.AbyssCreatureCharacterId, 2,
                    CardType.Attack, null, CardRarity.Rare,
                    Action(EffectActionType.DealDamage, EffectTarget.RandomEnemy, 4,
                        hitCount: 5)),
                PiercingTentacle = SaveCard("m_piercing_tentacle", "贯穿之触手", MinionTraitCatalog.AbyssCreatureCharacterId, 2,
                    CardType.Attack, null, CardRarity.Rare,
                    Action(EffectActionType.DealDamage, EffectTarget.DefaultEnemy, 17,
                        reach: TargetReach.Any),
                    Action(EffectActionType.DealTrueDamagePerStatusStack, EffectTarget.DefaultEnemy, 0,
                        statusId: StatusCatalog.Poison, stacks: 1, reach: TargetReach.Any)),
                GiantClaw = SaveCard("m_giant_claw", "巨钳击", "char_corrupted_crab", 2, CardType.Attack, null,
                    CardRarity.Rare,
                    Action(EffectActionType.DealDamage, EffectTarget.DefaultEnemy, 15)),
                ReforgeShell = SaveCard("m_reforge_shell", "重塑外壳", "char_corrupted_crab", 2, CardType.Defense, null,
                    Action(EffectActionType.GainBlock, EffectTarget.Self, 13)),
                PinchArmor = SaveCard("m_pinch_armor", "夹断护甲", "char_corrupted_crab", 2, CardType.Status, null,
                    CardRarity.Rare,
                    Action(EffectActionType.StripBlockThenDealDamage, EffectTarget.DefaultEnemy, 0,
                        stacks: 0, reach: TargetReach.Any)),
                FesterClaw = SaveCard("m_fester_claw", "溃烂钳击", "char_corrupted_crab", 2, CardType.Attack,
                    null, CardRarity.Rare,
                    Action(EffectActionType.DealDamage, EffectTarget.DefaultEnemy, 9,
                        reach: TargetReach.FrontAndMiddle),
                    Action(EffectActionType.DoubleAllDebuffStacksAndDuration, EffectTarget.DefaultEnemy, 0,
                        reach: TargetReach.FrontAndMiddle)),
                PhantomSlash = SaveCard("m_phantom_slash", "鬼魅斩击", "char_phantom_captain", 1, CardType.Attack, null,
                    Action(EffectActionType.DealDamage, EffectTarget.DefaultEnemy, 12)),
                MusketShot = SaveCard("m_musket_shot", "火枪射击", "char_phantom_captain", 1, CardType.Attack, null,
                    Action(EffectActionType.DealDamage, EffectTarget.DefaultEnemy, 12,
                        reach: TargetReach.BackOnly)),
                PhantomArmor = SaveCard("m_phantom_armor", "鬼灵盾甲", "char_phantom_captain", 2, CardType.Defense, null,
                    Action(EffectActionType.GainBlock, EffectTarget.Self, 11)),
                PlunderStrike = SaveCard("m_plunder_strike", "掠夺鬼击", "char_phantom_captain", 2, CardType.Attack, null,
                    CardRarity.Rare,
                    Action(EffectActionType.StealAllBuffs, EffectTarget.DefaultEnemy, 0, reach: TargetReach.Any),
                    Action(EffectActionType.DealDamage, EffectTarget.DefaultEnemy, 12, reach: TargetReach.Any)),
                PlunderCannon = SaveCard("m_plunder_cannon", "掠夺火炮", "char_phantom_captain", 3, CardType.Attack,
                    Kw("exhaust", "aoe"), CardRarity.Rare,
                    Action(EffectActionType.StealAllBuffs, EffectTarget.AllEnemies, 0, reach: TargetReach.Any),
                    Action(EffectActionType.DealDamage, EffectTarget.AllEnemies, 22, reach: TargetReach.Any)),
                Plunder = SaveCard("m_plunder", "劫掠", "char_phantom_captain", 1, CardType.Status, null,
                    CardRarity.Rare,
                    Action(EffectActionType.StealAllBlockFromRandomEnemyPreferArmored, EffectTarget.Self, 0)),
                GhostShip = SaveCard("m_ghost_ship", "驾驶幽灵船", "char_phantom_captain", 4, CardType.Status,
                    Kw("exhaust"), CardRarity.Epic,
                    Action(EffectActionType.ClearAllDebuffs, EffectTarget.Self, 0),
                    Action(EffectActionType.ApplyStatus, EffectTarget.Self, 0,
                        statusId: StatusCatalog.SpeedUp, stacks: 5, duration: -1),
                    Action(EffectActionType.ApplyStatus, EffectTarget.Self, 0,
                        statusId: StatusCatalog.DebuffImmune, stacks: 1, duration: -1))
            };
    }
}
#endif
